...
struct TrackJourneyStepTests {
	...
	struct SingleStepPatternTests {
		@Test
		func `Track records step in journey repository`() async {
		...
		@Test
		func `Track increments counter when single-step pattern matches`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
		...
		@Test
		func `Track increments counter only for matching step name`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policy = TestFactory.createPolicy(
				id: "test_policy",
		...
		@Test
		func `Track increments counters for multiple policies on same step`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
	...
	struct MultiStepPatternTests {
		@Test
		func `Track increments counter when strict sequence completes`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policy = TestFactory.createPolicy(
		...
		@Test
		func `Track does not increment counter when strict sequence has intermediate step`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policy = TestFactory.createPolicy(
				id: "strict_policy",
		...
		@Test
		func `Track increments counter when loose sequence completes`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policy = TestFactory.createPolicy(
				id: "loose_policy",
		...
		@Test
		func `Track prevents double-counting same sequence completion`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policy = TestFactory.createPolicy(
				id: "sequence_policy",
		...
		@Test
		func `Track counts multiple distinct sequence completions after history clear`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policy = TestFactory.createPolicy(
				id: "sequence_policy",
	...
	struct EdgeCaseTests {
		@Test
		func `Track records step but does not increment when no policies match`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policy = TestFactory.createPolicy(
				id: "policy",
		...
		@Test
		func `Track records step when policy list is empty`() async {
			let journeyRepo = InMemoryJourneyStepRepository()
			let stateRepo = InMemoryEventStateRepository()
			let policyProvider = MockPolicyProvider(policies: [])
			let matcher = SequenceMatcher()
